...
public class PlantingRecipeHandler extends MekanismRecipeHandler<PlantingRecipe> {
    ...
    @Override
    public String dumpToCommandString(IRecipeManager<? super PlantingRecipe> manager, PlantingRecipe recipe) {
        boolean hasSecondary = recipe.getSecondaryChance() > 0;
        List<ItemStack> mainOutputDefinition = recipe.getMainOutputDefinition();
        return buildCommandString(manager, recipe, recipe.getItemInput(), recipe.getGasInput(),
    ...
    public <U extends Recipe<?>> boolean doesConflict(IRecipeManager<? super PlantingRecipe> manager, PlantingRecipe recipe, U o) {
        if (o instanceof PlantingRecipe other) {
            return ingredientConflicts(recipe.getItemInput(), other.getItemInput()) &&
                    chemicalIngredientConflicts(recipe.getGasInput(), other.getGasInput());
        }
        return false;
    }

    @Override
    public Optional<IDecomposedRecipe> decompose(IRecipeManager<? super PlantingRecipe> manager, PlantingRecipe recipe) {
        List<ItemStack> mainOutputDefinition = recipe.getMainOutputDefinition();
        ...
        DecomposedRecipeBuilder builder = IDecomposedRecipe.builder()
                .with(CrTRecipeComponents.ITEM.input(), recipe.getItemInput());
        builder.with(CrTRecipeComponents.GAS.input(), recipe.getGasInput());
    ...
    @Override
    public Optional<PlantingRecipe> recompose(IRecipeManager<? super PlantingRecipe> m, ResourceLocation name, IDecomposedRecipe recipe) {
        if (m instanceof PlantingRecipeManager manager) {
            ItemStackIngredient itemInput = recipe.getOrThrowSingle(CrTRecipeComponents.ITEM.input());
            GasStackIngredient gasInput = recipe.getOrThrowSingle(CrTRecipeComponents.GAS.input());
