...
public abstract class TileEntityChemicalToChemicalFactory<RECIPE extends MekanismRecipe<?>> extends TileEntityAdvancedFactoryBase<RECIPE> {
    ...
    public boolean inputProducesOutput(int process, @NotNull ChemicalResource fallbackInput, @NotNull IChemicalTank outputTank, boolean updateCache) {
    ...
    @Nullable
    protected RECIPE getRecipeForInput(int process, @NotNull ChemicalResource fallbackInput, @NotNull IChemicalTank outputTank, boolean updateCache) {
    ...
    @Nullable
    protected abstract RECIPE findRecipe(int process, @NotNull ChemicalResource fallbackInput, @NotNull IChemicalTank outputTank);
    ...
    protected void sortInventoryOrTank() {
        ...
        for (CCProcessInfo processInfo : processInfoSlots) {
            ...
            if (inputTank.isEmpty()) {
                ...
                CCRecipeProcessInfo<ChemicalResource, RECIPE> recipeProcessInfo = processes.computeIfAbsent(inputStack, CCRecipeProcessInfo::new);
                recipeProcessInfo.processes.add(processInfo);
                recipeProcessInfo.totalAmount += inputTank.amountAsLong();
        ...
        for (Map.Entry<ChemicalResource, CCRecipeProcessInfo<ChemicalResource, RECIPE>> entry : processes.entrySet()) {
            CCRecipeProcessInfo<ChemicalResource, RECIPE> recipeProcessInfo = entry.getValue();
    ...
    protected void addEmptyTanksAsTargets(Map<ChemicalResource, CCRecipeProcessInfo<ChemicalResource, RECIPE>> processes, List<CCProcessInfo> emptyProcesses) {
        for (Map.Entry<ChemicalResource, CCRecipeProcessInfo<ChemicalResource, RECIPE>> entry : processes.entrySet()) {
            CCRecipeProcessInfo<ChemicalResource, RECIPE> recipeProcessInfo = entry.getValue();
    ...
    protected void distributeItems(Map<ChemicalResource, CCRecipeProcessInfo<ChemicalResource, RECIPE>> processes) {
        for (Map.Entry<ChemicalResource, CCRecipeProcessInfo<ChemicalResource, RECIPE>> entry : processes.entrySet()) {
            CCRecipeProcessInfo<ChemicalResource, RECIPE> recipeProcessInfo = entry.getValue();
    ...
    protected static class CCRecipeProcessInfo<ITEM, RECIPE extends MekanismRecipe<?>> {

        private final List<CCProcessInfo> processes = new ArrayList<>();
        @Nullable
        private ToIntBiFunction<CCRecipeProcessInfo<ITEM, RECIPE>, TileEntityChemicalToChemicalFactory<RECIPE>> lazyMinPerTank;
        private final ITEM item;
        private RECIPE recipe;
        private long minPerTank = 1;
        private long totalAmount;

        public CCRecipeProcessInfo(ITEM item) {
            this.item = item;
        }
