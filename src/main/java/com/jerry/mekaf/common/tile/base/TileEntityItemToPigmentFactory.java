...
public abstract class TileEntityItemToPigmentFactory<RECIPE extends MekanismRecipe> extends TileEntityAdvancedFactoryBase<RECIPE> {

    private static final long MAX_CHEMICAL = 10_000;

    protected I2PProcessInfo[] processInfoSlots;
    AdvancedFactoryInputInventorySlot[] inputSlot;
    IPigmentTank[] outputTank;

    protected final List<IInventorySlot> inputItemSlots;
    public final List<IPigmentTank> outputPigmentTanks;

    protected TileEntityItemToPigmentFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state, List<RecipeError> errorTypes, Set<RecipeError> globalErrorTypes) {
        super(blockProvider, pos, state, errorTypes, globalErrorTypes);
        inputItemSlots = new ArrayList<>();
        outputPigmentTanks = new ArrayList<>();

        processInfoSlots = new I2PProcessInfo[tier.processes];
        for (int i = 0; i < tier.processes; i++) {
            processInfoSlots[i] = new I2PProcessInfo(i, inputSlot[i], outputTank[i]);
        }

        for (I2PProcessInfo info : processInfoSlots) {
            inputItemSlots.add(info.inputSlot());
            outputPigmentTanks.add(info.outputTank());
        }

        addSupported(TransmissionType.PIGMENT);

        ConfigInfo gasConfig = configComponent.getConfig(TransmissionType.PIGMENT);
        if (gasConfig != null) {
            gasConfig.addSlotInfo(DataType.OUTPUT, new PigmentSlotInfo(false, true, outputPigmentTanks));
    ...
    @Nullable
    protected RECIPE getRecipeForInput(int process, @NotNull ItemStack fallbackInput, @Nullable IPigmentTank outputTank, boolean updateCache) {
    ...
    @Nullable
    protected abstract RECIPE findRecipe(int process, @NotNull ItemStack fallbackInput, @NotNull IPigmentTank outputTanks);
    ...
    public void parseUpgradeData(@NotNull IUpgradeData upgradeData) {
        if (upgradeData instanceof ItemToPigmentUpgradeData data) {
            ...
            for (int i = 0; i < data.outputTanks.size(); i++) {
                outputPigmentTanks.get(i).setStack(data.outputTanks.get(i).getStack());
            }
    ...
    protected void sortInventoryOrTank() {
        Map<HashedItem, I2PRecipeProcessInfo> processes = new HashMap<>();
        List<I2PProcessInfo> emptyProcesses = new ArrayList<>();
        for (I2PProcessInfo processInfo : processInfoSlots) {
            ...
            if (inputSlot.isEmpty()) {
                ...
                I2PRecipeProcessInfo recipeProcessInfo = processes.computeIfAbsent(item, i -> new I2PRecipeProcessInfo());
        ...
        for (Map.Entry<HashedItem, I2PRecipeProcessInfo> entry : processes.entrySet()) {
            I2PRecipeProcessInfo recipeProcessInfo = entry.getValue();
            if (recipeProcessInfo.lazyMinPerSlot == null) {
                ...
                recipeProcessInfo.lazyMinPerSlot = () -> {
                    ...
                    I2PProcessInfo processInfo = recipeProcessInfo.processes.get(0);
    ...
    private void addEmptySlotsAsTargets(Map<HashedItem, I2PRecipeProcessInfo> processes, List<I2PProcessInfo> emptyProcesses) {
        for (Map.Entry<HashedItem, I2PRecipeProcessInfo> entry : processes.entrySet()) {
            I2PRecipeProcessInfo recipeProcessInfo = entry.getValue();
            ...
            int added = 0;
            List<I2PProcessInfo> toRemove = new ArrayList<>();
            for (I2PProcessInfo emptyProcess : emptyProcesses) {
    ...
    private void distributeItems(Map<HashedItem, I2PRecipeProcessInfo> processes) {
        for (Map.Entry<HashedItem, I2PRecipeProcessInfo> entry : processes.entrySet()) {
            I2PRecipeProcessInfo recipeProcessInfo = entry.getValue();
            int processCount = recipeProcessInfo.processes.size();
            if (processCount == 1) {
            ...
            for (int i = 0; i < processCount; i++) {
                I2PProcessInfo processInfo = recipeProcessInfo.processes.get(i);
    ...
    public record I2PProcessInfo(int process, @NotNull AdvancedFactoryInputInventorySlot inputSlot,
                                 @NotNull IPigmentTank outputTank) {}

    public static class I2PRecipeProcessInfo {

        private final List<I2PProcessInfo> processes = new ArrayList<>();
        @Nullable
