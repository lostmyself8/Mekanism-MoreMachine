...
public class TileEntityLargeAntiprotonicNucleosynthesizer extends TileEntityProgressMachine<NucleosynthesizingRecipe> implements IBoundingBlock,
    ...
    public TileEntityLargeAntiprotonicNucleosynthesizer(BlockPos pos, BlockState state) {
        ...
        gasInputHandler = InputHelper.getInputHandler(gasTank, CachedRecipe.OperationTracker.RecipeError.NOT_ENOUGH_SECONDARY_INPUT);
    ...
    public @NotNull <T> LazyOptional<T> getOffsetCapabilityIfEnabled(@NotNull Capability<T> capability, Direction side, @NotNull Vec3i offset) {
        return IBoundingBlock.super.getOffsetCapabilityIfEnabled(capability, side, offset);
    }

    @Override
    public boolean isOffsetCapabilityDisabled(@NotNull Capability<?> capability, Direction side, @NotNull Vec3i offset) {
        return IBoundingBlock.super.isOffsetCapabilityDisabled(capability, side, offset);
    }
}
